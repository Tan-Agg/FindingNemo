...
# from intent_merger import IntentMerger  # Save the artifact as intent_merger.py
...
if __name__ == "__main__":
    ...
    # Test queries - Mix of SQL and Japan queries
    test_queries = [
        # SQL queries (should hit generalist, then train specialist)
        "Write SQL to find top 10 customers by revenue",
        "Generate a SQL query for user analytics",
        "Create SQL for sales data",
        "Write database query for inventory",
        ...
        # Japan queries (should hit the existing japan_travel specialist!)
        "What's the best time to visit Japan for cherry blossoms?",
        "Can you recommend a 7-day itinerary for Tokyo and Kyoto?",
        "What's the proper etiquette when visiting Japanese temples?",
    ]
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        print("\n📊 RESULT:")
        print(f"Answer: {result['answer'][:200] if result['answer'] else 'No answer'}...")  # First 200 chars
        print(f"\nMetadata:")
        print(f"  Routed to: {result['metadata']['routed_to']}")
        print(f"  Latency: {result['metadata']['latency']}s")
        print(f"  Intent: {result['metadata']['intent_label']}")
    ...
    print("\n" + "="*60)
    print("MERGING DUPLICATE INTENTS")
    print("="*60)
    merger = IntentMerger(agent.embedding_service.model)
    result = merger.merge_intents_in_logs(agent.query_logger, dry_run=False)
    merger.print_merge_report(result)
